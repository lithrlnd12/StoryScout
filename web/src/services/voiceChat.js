...
class WebVoiceChat {
    constructor() {
        ...
        this.audioElements = new Map(); // Store audio elements
        this.pendingSignals = new Map(); // Store signals received before peer creation
    }
    async initialize(partyId, userId, displayName) {
                ...
                if (change.type === 'added') {
                    ...
                    if (data.signals && data.signals[this.userId]) {
                        ...
                        if (peer) {
                            ...
                            this.connectToPeer(participantUserId, data, false);
                        }
                    }
    ...
    connectToPeer(peerId, participant, initiator) {
    ...
    async sendSignalToPeer(peerId, signal) {
        if (!this.partyId || !this.userId)
            return;
        const db = getFirestore(getFirebaseApp());
        const signalRef = doc(db, 'watchParties', this.partyId, 'voiceSignals', this.userId);
...
export const createVoiceChatManager = () => {
